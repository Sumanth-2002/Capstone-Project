package com.ust.Billing_Service.generator;

import com.ust.Billing_Service.repository.CustomerRepository;
import com.ust.Billing_Service.repository.SalesRepRepository;
import org.hibernate.engine.spi.SharedSessionContractImplementor;
import org.hibernate.id.IdentifierGenerator;
import org.springframework.context.ApplicationContext;
import org.springframework.context.ApplicationContextAware;
import org.springframework.stereotype.Component;

import java.io.Serializable;
import java.util.UUID;

@Component
public class SalesRepIdGenerator implements IdentifierGenerator, ApplicationContextAware {
    private static ApplicationContext applicationContext;

    @Override
    public void setApplicationContext(ApplicationContext context) {
        applicationContext = context;
    }

    @Override
    public Serializable generate(SharedSessionContractImplementor session, Object object) {
        SalesRepRepository salesRepRepository = applicationContext.getBean(SalesRepRepository.class);
        int count = (int) salesRepRepository.count();
        return "SREP" + (count + 1) + UUID.randomUUID().toString().substring(0, 5).toUpperCase();
    }
}
